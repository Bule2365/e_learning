...
@section('content')
    <div class="container mt-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="fw-bold text-primary">Daftar Ujian di Kelas {{ $class->name }}</h2>
            <a href="{{ route('admin.exams.index') }}" class="btn btn-primary">
                <i class="bi bi-arrow-left"></i> Kembali Ke Daftar Kelas</a>
        </div>

        <div class="row">
            @foreach ($class->ujian as $exam)
                <div class="col-md-4 mb-4">
                    <a href="{{ route('admin.exams.studentsByExam', $exam->id) }}" class="text-decoration-none">
                        <div class="card shadow-lg border-0 rounded-3 hover-effect">
                            <div class="card-body d-flex justify-content-between align-items-center">
                                <div>
                                    <h5 class="fw-bold text-dark mb-0">{{ $exam->title }}</h5>
